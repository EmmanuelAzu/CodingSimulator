...
export async function POST(req) {
  try {
    ...
    const rl = rateLimit({ key: `gen:${ip}`, limit: 10, windowMs: 5 * 60 * 1000 });
    if (!rl.allowed) {
...
`.trim();
    ...
    if (!payload.questions || !Array.isArray(payload.questions)) {
      return NextResponse.json({ error: "Model response missing 'questions' array" }, { status: 500 });
    }
    ...
    payload.questions = payload.questions.slice(0, 5).map((q) => {
      ...
      if (!starterLooksOk(qq.starterCode, lang)) qq.starterCode = starterTemplate(lang);
    ...
    return NextResponse.json({ error: "Server error generating questions" }, { status: 500 });
  }
}
